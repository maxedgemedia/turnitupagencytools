...
# ── CARRIER DATABASE (placeholder — client fills in real tolerances) ──────────
...
CARRIERS = {
    "Carrier A – Final Expense Plus": {
        ...
        "tobacco_lookback_years": 0,       # no lookback — current users OK
...
with tab1:
    ...
    if run:
        ...
        for i, (name, score, flags) in enumerate(results[:3]):
            ...
            with st.expander(f"{medals[i]} {name}  —  Score: {score}/12", expanded=(i == 0)):
                ...
                if flags:
                    ...
                    st.success("✅ Clean match — no red flags")
        ...
        st.success("✅ Lookup saved to your history.")
